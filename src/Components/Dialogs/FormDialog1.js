...
import styles from './FormDialog1.module.css';
...
export default function FormDialog1(props) {
  ...
  const create = async (data) => {
    let response;
    try {
      response = await fetch('http://localhost:3000/api/curso1', {
        method: 'POST',
        headers: {
  ...
  return (
    <div>
      ...
      >
        <DialogTitle id="responsive-dialog-title">Curso de Estilismo</DialogTitle>
        <DialogContent className={styles.container}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
            ...
            {errors.correo && errors.correo.type === 'pattern' && <span className={styles.error}>Ingresar un Apellido Valido</span> }
